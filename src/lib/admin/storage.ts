...
import { dealsApi, leadsApi, renewalsApi, contentApi } from "./api";
...
export function useCollection<T extends Record<string, any>>(
  ...
  const [items, setItems] = useState<WithId<T>[]>(() => (key === 'wo_admin_deals' || key === 'wo_admin_leads' || key === 'wo_admin_renewals' || key === 'wo_admin_content') ? [] : read<T>(key, seed));
